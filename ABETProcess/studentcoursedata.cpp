...
bool StudentCourseData::ReadFromExcel(const QString &filename)
{
    ...
    while (xlsxR.cellAt(row,1)) {
        if (!xlsxR.cellAt(row, 1)->readValue().toString().isEmpty())
        {
            ...
            item.StudentID = xlsxR.cellAt(row, 1)->readValue().toString();
            item.StudentName = xlsxR.cellAt(row, 2)->readValue().toString();
            item.StrtLevel = xlsxR.cellAt(row, 3)->readValue().toString();
            item.AcadProg = xlsxR.cellAt(row, 4)->readValue().toString();
            item.AcadPlan = xlsxR.cellAt(row, 5)->readValue().toString();
            item.Subject = xlsxR.cellAt(row, 6)->readValue().toString();
            item.Catalog = xlsxR.cellAt(row, 7)->readValue().toString();
            item.Section = xlsxR.cellAt(row, 8)->readValue().toString();
            item.Title = xlsxR.cellAt(row, 9)->readValue().toString();
            item.unit = xlsxR.cellAt(row, 10)->readValue().toInt();
            if (xlsxR.cellAt(row, 11))
                item.RepeatCode = xlsxR.cellAt(row, 11)->readValue().toString();
            if (xlsxR.cellAt(row, 12))
                item.RepeatCodeDescr = xlsxR.cellAt(row, 12)->readValue().toString();
            if (xlsxR.cellAt(row, 13))
                item.GradeIn = xlsxR.cellAt(row, 13)->readValue().toString();
            if (xlsxR.cellAt(row, 14))
                item.OfficialGrade = xlsxR.cellAt(row, 14)->readValue().toString();
            if (xlsxR.cellAt(row, 15))
                item.GradeIn = xlsxR.cellAt(row, 15)->readValue().toString();
            if (xlsxR.cellAt(row, 16))
                item.GradingBasis = xlsxR.cellAt(row, 16)->readValue().toString();
